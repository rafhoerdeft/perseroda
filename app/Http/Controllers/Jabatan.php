...
class Jabatan extends UserBaseController
{
    protected $is_role;
    ...
    {
        $breadcrumb = ['Jabatan'];
        $form_title = 'Input Jabatan';

        $is_role = $this->is_role;
        $main_route = 'jabatan.';

        $list_data = ModelsJabatan::latest()->get();
        return view('pages/jabatan/list', compact(
            'breadcrumb',
            'form_title',
            'list_data',
            'is_role',
    ...
    public function store(Request $request)
    {
        ...
        try {
            if ($request->jabatan_id) {
                $id = decode($request->jabatan_id);

                $save = ModelsJabatan::find($id)->update($data);
            } else {
                $save = ModelsJabatan::create($data);
            }
    ...
    public function delete(Request $request)
    {
        ...
        try {
            ...
            $id = $request->id;
            $data_jabatan = ModelsJabatan::find(decode($id));

            $deleted = $data_jabatan->delete();
