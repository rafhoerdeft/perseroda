                    ...
                    <div class="row" id="jenis_bayar">
                        <div class="col-md-4">
                            <input class="btn-check" type="radio" name="jenis_bayar" id="tunai" value="tunai"
                                autocomplete="off"
                                {{ old('jenis_bayar') ?? (isset($order) ? ($order->jenis_bayar == 'tunai' ? 'checked' : '') : 'checked') }}>
                        ...
                        <div class="col-md-4">
                            <input class="btn-check" type="radio" name="jenis_bayar" id="bank" value="bank"
                                autocomplete="off"
                                {{ old('jenis_bayar') ?? (isset($order) ? ($order->jenis_bayar == 'bank' ? 'checked' : '') : '') }}>
                    ...
                    <div class="row" id="status_bayar">
                        <div class="col-md-4">
                            <input class="btn-check" type="radio" name="status_bayar" id="lunas" value="1"
                                autocomplete="off"
                                {{ old('status_bayar') ?? (isset($order) ? ($order->status_bayar == '1' ? 'checked' : '') : 'checked') }}>
                        ...
                        <div class="col-md-4">
                            <input class="btn-check" type="radio" name="status_bayar" id="belum_bayar" value="0"
                                autocomplete="off"
                                {{ old('status_bayar') ?? (isset($order) ? ($order->status_bayar == '0' ? 'checked' : '') : '') }}>
...
@push('js_script')
    ...
    <script>
        ...
        $('.select2').select2({
            ajax: {
                url: "{{ route('order.perdagangan.produk') }}",
                dataType: 'json',
                type: "GET",
                // quietMillis: 50,
                delay: 150,
    ...
    <script>
        function getDataProduk() {
            ...
            try {
                ...
                $.get("{{ route('order.perdagangan.produk') }}", {
                    'id': id
